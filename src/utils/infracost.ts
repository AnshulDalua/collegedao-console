...
export async function infracost(key: (string | boolean | number)[], query: {}) {
  const kedy = key.join();
  if (cache.has(kedy)) return cache.get(kedy) as Cost;

  const data = await wretch("/api/cost/plan")
...
export async function aws_storage(
  ...
  return infracost(["awsstorage", region, acl, versioning], {
    format_version: "1.2",
    terraform_version: "1.5.3",
    planned_values: {
      root_module: {
...
export async function gcp_storage(
  ...
  return infracost(["gcpstorage", region, acl, versioning], {
    format_version: "1.2",
    terraform_version: "1.5.3",
    planned_values: {
      root_module: {
